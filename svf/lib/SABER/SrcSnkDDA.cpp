...
void SrcSnkDDA::analyze(SVFModule* module)
{
   ...
   // initialize(module);//in module, stored the functionset, globalset and so on
    ...
    for (SVFGNodeSetIter iter = sourcesBegin(), eiter = sourcesEnd();
            iter != eiter; ++iter)
    {
        setCurSlice(*iter); //init _curSlice with svfg and so on
        ContextCond cxt;
 ...
 */
void SrcSnkDDA::FWProcessOutgoingEdge(const DPIm& item, SVFGEdge* edge)//but not analysis here, just propogate here
{
    ...
    if(pushIntoWorklist(newItem))
        ...
        std::cout<< " --> " << edge->getDstID() << ", cxt size: " << newItem.getContexts().cxtSize() <<")\n";
